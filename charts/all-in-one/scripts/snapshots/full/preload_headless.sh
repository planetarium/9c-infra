...
function senderr() {
  echo "$1"
  curl --data "[K8S] $1. Check snapshot-full-v$VERSION_NUMBER in {{ $.Values.clusterName }} cluster at preload_headless.sh." "https://planetariumhq.slack.com/services/hooks/slackbot?token=$SLACK_TOKEN&channel=%239c-mainnet"
}
