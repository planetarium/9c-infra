...
function download_partition() {
  function test_extension() {
    ...
    for ext in "${EXTENSIONS[@]}"; do
      curl -I "$base_url/$filename.$ext" > /dev/null 2>&1
      if [ $? -eq 0 ]; then
        echo "$ext"
        return 0
      fi
      curl -I $(printf "$base_url/$filename.$ext.part%0${part_length}d" 1) > /dev/null 2>&1
      if [ $? -eq 0 ]; then
        echo "$ext.parted"
        return 0
      fi
...
if [ $download_option = "true" ]; then
  ...
  function download_unzip_snapshot() {
    ...
    for ((i=${#snapshot_zip_filename_array[@]}-1; i>=0; i--)); do
      ...
      download_partition "$base_url" "$snapshot_zip_filename" "$save_dir"
      echo "Unzipping $snapshot_zip_filename"
      bsdtar -xf "$save_dir/$snapshot_zip_filename.*" -C "$save_dir"
